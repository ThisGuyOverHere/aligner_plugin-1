module.exports = function (grunt) {
            ...
            dist: {
                ...
                options: {
                    replacements: [
                        {
                            pattern: 'GA_FROM_INI###',
                            replacement: config.GA_UA ? config.GA_UA : ''
                        },
                        {
                            pattern: 'SENTRY_DSN_FROM_INI###',
                            replacement: config.SENTRY_DSN ? config.SENTRY_DSN : ''
                        },
                        {
                            pattern: 'SENTRY_ENVIRONMENTS_FROM_INI###',
                            replacement: config.SENTRY_ENVIRONMENTS ? config.SENTRY_ENVIRONMENTS : ''
                        }
                    ]
                }
            }
    ...
    grunt.registerTask('default', ['clean:build', 'env:prod', 'string-replace', 'copy', 'browserify:dist', 'browserify:matecatDist', 'uglify:dist', 'sass', 'autoprefixer']);
    grunt.registerTask('matecat', ['browserify:matecatDist']);
    grunt.registerTask('dev', ['env:dev', 'clean:build', 'copy', 'string-replace', 'browserify:dev', 'browserify:matecatDist', 'sass', 'autoprefixer', 'watch']);
